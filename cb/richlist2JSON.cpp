...
struct richlist2JSON:public Callback
{
    bool detailed;
    int64_t limit;
    uint64_t offset;
    ...
    virtual const char                   *name() const         { return "richlist2JSON"; }
    ...
    virtual void wrapup()
    {
        ...
        while(likely(s<e)) {
            ...
            printf("{\"balance\": \"%f", (1e-8)*addr->sum);
			printf("\", ");
		...
		printf(
            "],\n");
		strftime (buffer,80,"%I:%M%p on %D",timeinfo);
		printf ("\"updatetime\": \"%s\"\n}", buffer);
...
static richlist2JSON richlist2JSON;
